...
export const AnalyticsNode: React.FC<NodeProps> = ({ id, data, selected }) => {
  const { deleteNode } = useFlow();
  const onConfigClick = data.onConfigClick as ((nodeId: string) => void) | undefined;

  const metrics = (data.metrics as string[]) || [];
  const reportingInterval = (data.reportingInterval as string) || 'hourly';
  const dashboard = (data.dashboard as string) || '';
  
  // Check if node has any configuration
  const hasConfiguration = metrics.length > 0 || dashboard;

  const intervalColors = {
    realtime: 'bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-400',
    minutely: 'bg-orange-100 text-orange-800 dark:bg-orange-900/20 dark:text-orange-400',
    ...
    `}>
      ...
      <div className="flex items-center gap-2 mb-2">
        <div className={`${hasConfiguration ? 'p-1.5' : 'p-1'} rounded-md bg-primary/10`}>
          <BarChart3 className={`${hasConfiguration ? 'w-4 h-4' : 'w-3 h-3'} text-primary`} />
        </div>
        {hasConfiguration && (
          <div className="flex-1 min-w-0">
            <div className="text-sm font-medium truncate text-primary">Analytics</div>
        ...
        {!hasConfiguration && (
          <div className="flex-1 text-center">
            <h3 className="font-medium text-xs text-primary">Analytics</h3>
          </div>
        )}
      </div>
      ...
      {hasConfiguration && (
        <div className="space-y-2 mb-3">
          ...
          {/* Reporting Interval Block */}
          <div className="bg-accent/30 rounded p-2">
            <div className="flex items-center justify-between">
              <span className="text-xs font-medium">Interval</span>
              <Badge 
                variant="secondary"
                className={`text-xs ${intervalColors[reportingInterval as keyof typeof intervalColors] || intervalColors.hourly}`}
              >
                {reportingInterval}
              </Badge>
            </div>
