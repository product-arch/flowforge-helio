...
import { Webhook, Trash2, Settings } from 'lucide-react';
...
export const WebhookNode: React.FC<NodeProps> = ({ id, data, selected }) => {
  ...
  const hasConfiguration = url || (headers && Object.keys(headers).length > 0);
    ...
    `}>
      ...
      <div className="flex items-center gap-2 mb-2">
        <div className={`${hasConfiguration ? 'p-1.5' : 'p-1'} rounded-md bg-primary/10`}>
          <Webhook className={`${hasConfiguration ? 'w-4 h-4' : 'w-3 h-3'} text-primary`} />
        </div>
        {hasConfiguration && (
          <div className="flex-1 min-w-0">
            <div className="text-sm font-medium truncate text-primary">Webhook</div>
        ...
        {!hasConfiguration && (
          <div className="flex-1 text-center">
            <h3 className="font-medium text-xs text-primary">Webhook</h3>
          </div>
        )}
      </div>
        ...
        <div className="space-y-2 mb-3">
          ...
          <div className="bg-accent/30 rounded p-2">
            <div className="flex items-center justify-between">
              ...
              <Badge 
                variant="secondary"
                className={`text-xs ${methodColors[method as keyof typeof methodColors] || methodColors.POST}`}
              >
                {method}
              </Badge>
            </div>
