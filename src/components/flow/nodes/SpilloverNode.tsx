...
import { TrendingDown, Trash2, Settings } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { useFlow } from '@/contexts/FlowContext';

export const SpilloverNode: React.FC<NodeProps> = ({ id, data, selected }) => {
  const { deleteNode } = useFlow();
  const onConfigClick = data.onConfigClick as ((nodeId: string) => void) | undefined;

  const vendors = (data.vendors as any[]) || [];
  const spilloverMode = (data.spilloverMode as string) || 'capacity_based';
    ...
    `}>
      ...
      <div className="flex items-center gap-2 mb-2">
        <div className={`${hasConfiguration ? 'p-1.5' : 'p-1'} rounded-md bg-primary/10`}>
          <TrendingDown className={`${hasConfiguration ? 'w-4 h-4' : 'w-3 h-3'} text-primary`} />
        </div>
        {hasConfiguration && (
          <div className="flex-1 min-w-0">
            <div className="text-sm font-medium truncate text-primary">Spillover</div>
        ...
        {!hasConfiguration && (
          <div className="flex-1 text-center">
            <h3 className="font-medium text-xs text-primary">Spillover</h3>
          </div>
        )}
      </div>
        ...
        <div className="space-y-2 mb-3">
          ...
          <div className="bg-accent/30 rounded p-2">
            <div className="flex items-center justify-between">
              ...
              <Badge 
                variant="secondary"
                className={`text-xs ${modeColors[spilloverMode as keyof typeof modeColors] || modeColors.capacity_based}`}
              >
                {spilloverMode.replace('_', ' ')}
              </Badge>
            </div>
          </div>
