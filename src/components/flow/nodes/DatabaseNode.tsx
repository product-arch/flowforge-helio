...
export const DatabaseNode: React.FC<NodeProps> = ({ id, data, selected }) => {
  ...
  const connectionString = (data.connectionString as string) || '';
  const operation = (data.operation as string) || 'select';
  const table = (data.table as string) || '';
  
  // Check if node has any configuration
  const hasConfiguration = connectionString || table;

  const operationColors = {
    select: 'bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-400',
    insert: 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400',
    update: 'bg-orange-100 text-orange-800 dark:bg-orange-900/20 dark:text-orange-400',
    ...
    `}>
      ...
      <div className="flex items-center gap-2 mb-2">
        <div className={`${hasConfiguration ? 'p-1.5' : 'p-1'} rounded-md bg-primary/10`}>
          <Database className={`${hasConfiguration ? 'w-4 h-4' : 'w-3 h-3'} text-primary`} />
        </div>
        {hasConfiguration && (
          <div className="flex-1 min-w-0">
            <div className="text-sm font-medium truncate text-primary">Database</div>
            <div className="text-xs text-muted-foreground truncate">
              {table || 'No table configured'}
            </div>
          </div>
        )}
        {!hasConfiguration && (
          <div className="flex-1 text-center">
            <h3 className="font-medium text-xs text-primary">Database</h3>
          </div>
        )}
      </div>
        ...
        <div className="space-y-2 mb-3">
          ...
          <div className="bg-accent/30 rounded p-2">
            <div className="flex items-center justify-between">
              ...
              <Badge 
                variant="secondary"
                className={`text-xs ${operationColors[operation as keyof typeof operationColors] || operationColors.select}`}
              >
