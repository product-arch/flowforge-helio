...
import { DollarSign, Trash2, Settings } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { useFlow } from '@/contexts/FlowContext';

export const NewLeastCostNode: React.FC<NodeProps> = ({ id, data, selected }) => {
  ...
  return (
    ...
    `}>
      ...
      <div className="flex items-center gap-2 mb-3">
        <div className="p-1.5 rounded-md" style={{ backgroundColor: 'rgba(234, 179, 8, 0.1)' }}>
          <DollarSign className="w-4 h-4" style={{ color: 'rgb(234, 179, 8)' }} />
        </div>
        <div className="flex-1 min-w-0">
          <div className="text-sm font-medium truncate text-primary">Least Cost</div>
          {connectedChannel && (
      ...
      {displayMessage ? (
        ...
        <div className="space-y-2">
          ...
          {data.fallbackToHigherCost && (
            <div className="bg-status-warning/10 rounded p-1">
              <span className="text-xs text-status-warning font-medium">⚡ Fallback Enabled</span>
            </div>
          )}
        </div>
      ) : (
