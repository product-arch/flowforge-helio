...
export const GeolocationNode: React.FC<NodeProps> = ({ id, data, selected }) => {
  const { deleteNode } = useFlow();
  const onConfigClick = data.onConfigClick as ((nodeId: string) => void) | undefined;

  const regions = (data.regions as any[]) || [];
  const geoStrategy = (data.geoStrategy as string) || 'country_based';
  const defaultRegion = (data.defaultRegion as string) || 'global';
  
  // Check if node has configuration
  const hasConfiguration = regions.length > 0 || defaultRegion !== 'global';

  const strategyColors = {
    country_based: 'bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-400',
    region_based: 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400',
    timezone_based: 'bg-purple-100 text-purple-800 dark:bg-purple-900/20 dark:text-purple-400',
    latency_based: 'bg-orange-100 text-orange-800 dark:bg-orange-900/20 dark:text-orange-400',
    compliance_based: 'bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-400'
  };
    ...
    `}>
      ...
      <div className="flex items-center gap-2 mb-2">
        <div className={`${hasConfiguration ? 'p-1.5' : 'p-1'} rounded-md bg-primary/10`}>
          <MapPin className={`${hasConfiguration ? 'w-4 h-4' : 'w-3 h-3'} text-primary`} />
        </div>
        {hasConfiguration && (
          <div className="flex-1 min-w-0">
            <div className="text-sm font-medium truncate text-primary">Geo Routing</div>
        ...
        {!hasConfiguration && (
          <div className="flex-1 text-center">
            <h3 className="font-medium text-xs text-primary">Geo Routing</h3>
          </div>
        )}
      </div>
        ...
        <div className="space-y-2 mb-3">
          ...
          <div className="bg-accent/30 rounded p-2">
            <div className="flex items-center justify-between">
              ...
              <Badge 
                variant="secondary"
                className={`text-xs ${strategyColors[geoStrategy as keyof typeof strategyColors] || strategyColors.country_based}`}
              >
                {geoStrategy.replace('_', ' ')}
              </Badge>
            </div>
          </div>
